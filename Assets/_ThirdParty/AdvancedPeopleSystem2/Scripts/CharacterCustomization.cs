...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        /// </summary>
        public void EditorSavePreBuiltPrefab(bool bakeBlendshapes = false, int materialInstType = 0)
        {
#if UNITY_EDITOR
                ...
                CharacterCustomizationCombiner.MakeCombinedMeshes(this, bakeBlendshapes, prebuiltPrefab, 0.06f, (newMeshes) =>
                {
                    try
                    {
                        ...
                        foreach (Material mat in allMeshes[0].sharedMaterials)
                        {
                            ...
                            if ((mat.shader.name == Settings.bodyMaterial.shader.name && materialInstType == 2) || materialInstType == 0)
                            {
        ...
        public void ForceStopBlendshapeAnimation(string animationName)
        {
            currentBlendshapeAnimation = null;
            var bAnimPreset = selectedsettings.characterAnimationPresets.Where(x=>x.name == animationName).FirstOrDefault();
            if(bAnimPreset != null)
            {            
                for (var i = 0; i < bAnimPreset.blendshapes.Count; i++)
                {
                    var bData = GetBlendshapeData(bAnimPreset.blendshapes[i].BlendType);
                    SetBlendshapeValue(bData.type, 0f);
                }
            }
        }
